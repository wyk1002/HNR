...
def getSLastKEvent(history,last_k):
    s_history_oid = []
    last_kEvents=[]
    for oneEvent in history:
        s_history_oid.append([])
        last_kEvents.append([])
        for con_events in oneEvent:
            s_history_oid[-1] += con_events[:, 1].tolist()
            last_kEvents[-1] += con_events.tolist()
        last_kEvents[-1]=np.asarray(last_kEvents[-1][-last_k:],dtype=int)
    return s_history_oid,last_kEvents


def getOLastKEvent(history,last_k):
    o_history_sid = []
    last_kEvents = []
    for oneEvent in history:
        ...
        for con_events in oneEvent:
            ...
            last_kEvents[-1]+=con_events.tolist()
        last_kEvents[-1]=np.asarray(last_kEvents[-1][-last_k:],dtype=int)
    return o_history_sid,last_kEvents

def getNoneLastKEvent(history,last_k):
    o_history_sid = []
    last_kEvents = []
    for oneEvent in history:
        o_history_sid.append([])
...
def make_batch(a, b, c, d, e, f, g,h,i, batch_size, valid1=None, valid2=None):
    if valid1 is None and valid2 is None:
        for idx in range(0, len(a), batch_size):
            yield [a[idx:idx + batch_size], b[idx:idx + batch_size], c[idx:idx + batch_size], d[idx:idx + batch_size],
                   e[idx:idx + batch_size], f[idx:idx + batch_size], g[idx:idx + batch_size], h[idx:idx + batch_size],
                   i[idx:idx + batch_size]]
    else:
        for idx in range(0, len(a), batch_size):
            yield [a[idx:idx + batch_size], b[idx:idx + batch_size], c[idx:idx + batch_size], d[idx:idx + batch_size],
                   e[idx:idx + batch_size], f[idx:idx + batch_size], g[idx:idx + batch_size], h[idx:idx + batch_size],
