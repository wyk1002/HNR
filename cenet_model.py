...
class CENET(nn.Module):
    def __init__(self, num_e, num_rel, num_t, args):
        ...
        self.W_satt=nn.Linear(args.embedding_dim, 1)
        self.W_oatt = nn.Linear(args.embedding_dim, 1)
        self.oracle_layer = Oracle(3 * args.embedding_dim, 1)
        self.oracle_layer.apply(self.weights_init)

        self.ro2s = nn.Linear(3*args.embedding_dim, args.embedding_dim)
        self.rs2o = nn.Linear(3*args.embedding_dim, args.embedding_dim)
        ...
        self.weights_init(self.W_satt)
        self.weights_init(self.W_oatt)
        self.weights_init(self.ro2s)
        self.weights_init(self.rs2o)
    ...
    def forward(self, batch_block, mode_lk, total_data=None):
        # quadruples, s_history_oid, o_history_sid, \
        # s_history_label_true, o_history_label_true, s_frequency, o_frequency, s_last_Event,o_last_Event = batch_block

        quadruples,s_history_label_true,o_history_label_true,s_frequency,o_frequency,s_history_oid,o_history_sid,s_last_Event,o_last_Event=batch_block

        if isListEmpty(s_history_oid) or isListEmpty(o_history_sid):
        ...
        s_local_tag=copy.deepcopy(s_frequency)
        o_local_tag=copy.deepcopy(o_frequency)
        ...
        s_r,s_o=s_last_Event[:,0],s_last_Event[:,1]
        o_r, o_s = o_last_Event[:, 0], o_last_Event[:, 1]

        s_last_mask = torch.tensor((s_r != -1)).unsqueeze(-1).to(s_frequency_hidden.device)

        local_S_features = self.ro2s(torch.cat([self.entity_embeds[s],self.rel_embeds[s_r]*s_last_mask, self.entity_embeds[s_o]*s_last_mask], dim=-1))
        # local_S_features = self.ro2s(self.entity_embeds[s]+self.rel_embeds[s_r] * s_last_mask+self.entity_embeds[s_o] * s_last_mask)

        # local_S_features = local_s_feature*s_last_mask.unsqueeze(-1)

        # (b,k,d)
        o_last_mask = torch.tensor((o_r != -1)).unsqueeze(-1).to(o_frequency_hidden.device)
        local_O_features = self.rs2o(torch.cat([self.entity_embeds[o],self.rel_embeds[-self.num_rel:][o_r]*o_last_mask,self.entity_embeds[o_s]*o_last_mask],
                                              dim=-1))
        # local_O_features = self.rs2o(self.entity_embeds[o]+self.rel_embeds[o_r] * o_last_mask+self.entity_embeds[o_s] * o_last_mask)

        # local_O_features = local_o_feature * o_last_mask.unsqueeze(-1)
        ...
        if mode_lk == 'Training':
            s_nce_loss, _ = self.calculate_nce_loss(s, o, r, self.rel_embeds[:self.num_rel],
                                                    ...
                                                    s_history_tag, s_non_history_tag,s_local_tag,local_S_features,self.W_satt)
            o_nce_loss, _ = self.calculate_nce_loss(o, s, r, self.rel_embeds[-self.num_rel:],
                                                    ...
                                                    o_history_tag, o_non_history_tag,o_local_tag,local_O_features,self.W_oatt)
    ...
    def calculate_nce_loss(self, actor1, actor2, r, rel_embeds, linear1, linear2,linear3, history_tag, non_history_tag, local_tag, local_features, W_att):
        b=actor1.shape[0]
        ...
        local_raw=self.tanh(linear3(self.dropout(torch.cat((self.entity_embeds[actor1], rel_embeds[r]), dim=1)))+local_features)

        local_preds=F.softmax(local_raw.mm(self.entity_embeds.transpose(0, 1))+local_tag)

        states = torch.cat(
            (preds_raw1.view(b, 1, -1), preds_raw2.view(b, 1, -1),local_raw.view(b, 1, -1),), dim=1)
        att = F.softmax(W_att(states), dim=1).view(b, 1, -1)
        dist = torch.cat((preds1.view(b, 1, -1), preds2.view(b, 1, -1), local_preds.view(b, 1, -1)),
                         dim=1)
        predict_summary = torch.matmul(att, dist)
        predict_summary = predict_summary.view(b, -1)
        # predict_summary = preds1+preds2+local_preds

        # cro_entr_loss = self.criterion_link(preds1 + preds2, actor2)

        nce = torch.sum(torch.gather(torch.log(predict_summary), 1, actor2.view(-1, 1)))
        nce /= -1. * actor2.shape[0]

        pred_actor2 = torch.argmax(predict_summary, dim=1)  # predicted result
        correct = torch.sum(torch.eq(pred_actor2, actor2))
        accuracy = 1. * correct.item() / actor2.shape[0]
        print('# Batch accuracy', accuracy)

        return nce, predict_summary
    ...
    def freeze_parameter(self):
        ...
        self.ro2s.requires_grad_(False)
        self.rs2o.requires_grad_(False)
        self.W_satt.requires_grad_(False)
        self.W_oatt.requires_grad_(False)
